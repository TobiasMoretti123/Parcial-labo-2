...
static int addNode(LinkedList *this, int nodeIndex, void *pElement) {
	...
	if (this != NULL) {
		...
		if (nodeIndex >= 0 && nodeIndex <= len) {
			...
			if (nuevoNodo != NULL) {
				if (nodeIndex == 0) {
					nodoAnterior = getNode(this, 0);
					this->pFirstNode = nuevoNodo;
					nuevoNodo->pNextNode = nodoAnterior;
				} else {
...
int ll_containsAll(LinkedList *this, LinkedList *this2) {
	...
	if (this != NULL && this2 != NULL) {
		...
		returnAux = 1;
		for (i = 0; i < len; i++) {
			contains = ll_contains(this, ll_get(this2, i));
			if (contains == 0) {
				returnAux = 0;
				break;
			}
	...
	if (this != NULL && pFunc != NULL && (order == 0 || order == 1)) {
		...
		for (i = 0; i < len; i++) {
			for (j = i + 1; j < len; j++) {
				pAux = ll_get(this, i);
				pAux2 = ll_get(this, j);
				if ((order == 0 && pFunc(pAux, pAux2) < 0)
						|| (order == 1 && pFunc(pAux, pAux2) > 0)) {
					ll_set(this, j, pAux);
					ll_set(this, i, pAux2);
				}
			}
...
LinkedList* ll_filter(LinkedList *this, int (*fn)(void *element)) {
	LinkedList *cloneArraylist;
	void *elemento;
	int len;
	if (this != NULL && fn != NULL) {
		len = ll_len(this);
		cloneArraylist = ll_newLinkedList();
		if (cloneArraylist != NULL) {
			for (int i = 0; i < len; i++) {
				elemento = ll_get(this, i);
				if (fn(elemento) == 1) {
					ll_add(cloneArraylist, elemento);
				}
			}
		}
	}
